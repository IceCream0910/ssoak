...
export default function Upload() {
    ...
    const handle과세특EditChange = (grade, category, index, e) => {
        ...
        set과세특JSON((prev) => {
            ...
            updatedData[grade][category][index] = e.value;
            ...
            <main className={`${styles.main}`}>
                ...
                <header>
                    <div className="header-left">
                        <IonIcon name='chevron-back-outline' onClick={() => router.back()} /> <h3 className="header-title">내 생기부</h3>
                    </div>
                ...
                <div className="sanggibu-card" style={{ padding: "15px 20px", textAlign: 'center', }}>
                    ...
                    {(name && 자동진JSON && 과세특JSON) && <>
                        <p>⚠️ 반드시 저장을 눌러주세요.</p>
                        <p>- 아래 항목을 확인하고 수정이 필요한 경우 수정 후 저장해주세요.</p>
                ...
                {(name && 자동진JSON && 과세특JSON) ? <>
                    ...
                    {자동진JSON &&
                        자동진JSON.map((item, index) => {
                            return (
                                <div key={index} className="jadongjin-card">
                                        ...
                                        return (
                                            <div key={category}>
                                                {과세특JSON[grade][category].map((item, index) => {
                                                    ...
                                                    return (
                                                        <div key={index} className="sanggibu-card">
                                                            <textarea
                                                                value={item.content}
